...
async function QuestionStudent({question = {title, content, id}, awnsers = [{content, id, selected}], indice = null}){
...
async function checkIfQuizIsToStudent(){
    ...
    const userIdQuiz = attemptReq.attempt.aluno_id
...
async function quizStartPage(){
    ...
    for(const question of req.quizz.perguntas){
        ...
        questionsDiv.append(await QuestionStudent({question: questionObject, awnsers: awnserArray, indice: indice}))
        indice += 1
    }
